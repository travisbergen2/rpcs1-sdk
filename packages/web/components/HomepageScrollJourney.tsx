...
const STAGES: Array<{
...
}> = [
  {
    ...
    body: 'The page opens flat and legible: buttons, links, and a simple promise.',
    detail: 'You know what RPCS-1 is, who it is for, and what to do next before anything gets dramatic.',
  },
  {
    key: 'bridge',
    label: 'stage 2',
    title: 'The interface starts reasoning',
    body: 'The browser gives way to a signal field, and the product starts explaining itself through motion.',
    detail: 'Connections appear, assumptions get labeled, and the user sees why the system thinks a workflow will fail.',
  },
  {
    key: 'room',
    label: 'stage 3',
    title: 'Face to face with the sales room',
    body: 'The scroll ends in a 3D room with an avatar and the deliverables presented as artifacts.',
...
function BridgeScene() {
  return (
    <div className="relative h-full overflow-hidden rounded-[2rem] border border-white/10 bg-[#0d1220] text-white shadow-[0_30px_120px_rgba(0,0,0,0.55)]">
      <div
        className="pointer-events-none absolute inset-0 opacity-90"
        style={{
          backgroundImage:
            'radial-gradient(circle at 15% 20%, rgba(56,189,248,0.20), transparent 18%), radial-gradient(circle at 85% 22%, rgba(168,85,247,0.16), transparent 18%), radial-gradient(circle at 50% 65%, rgba(16,185,129,0.08), transparent 20%), linear-gradient(rgba(255,255,255,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.04) 1px, transparent 1px)',
          backgroundSize: '100% 100%, 100% 100%, 100% 100%, 3.5rem 3.5rem, 3.5rem 3.5rem',
      ...
      <div className="relative grid h-full gap-4 p-4 sm:grid-cols-[0.95fr_1.05fr]">
        <div className="rounded-[1.6rem] border border-white/10 bg-white/5 p-4 backdrop-blur-xl">
          ...
          <p className="mt-3 text-sm leading-relaxed text-white/60">
            The site is no longer a brochure. It is a working space where the system labels uncertainty, resolves ambiguity, and shows the next check.
          </p>
          <div className="mt-4 space-y-2">
            {[
              'Less clarification churn',
              'More usable corrections',
              'Cleaner handoffs',
            ].map((item) => (
              <div key={item} className="rounded-2xl border border-white/10 bg-black/20 px-3 py-2 text-sm text-white/75">
                {item}
              </div>
            ))}
          </div>
        ...
        <div className="relative rounded-[1.6rem] border border-white/10 bg-white/5 p-4 backdrop-blur-xl">
          ...
          <div className="relative mt-4 h-[calc(100%-1.75rem)] overflow-hidden rounded-[1.4rem] border border-white/10 bg-[#06111d]">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_25%,rgba(56,189,248,0.18),transparent_22%),radial-gradient(circle_at_20%_70%,rgba(168,85,247,0.12),transparent_18%),radial-gradient(circle_at_80%_65%,rgba(16,185,129,0.08),transparent_18%)]" />
            ...
            <div className="absolute left-1/2 top-14 h-24 w-px bg-gradient-to-b from-sky-300/80 to-transparent" />
            ...
            <div className="absolute left-[26%] top-[43%] h-20 w-20 rounded-[1.5rem] border border-fuchsia-400/20 bg-fuchsia-500/10 blur-[0.5px]" />
            <div className="absolute right-[24%] top-[39%] h-20 w-20 rounded-[1.5rem] border border-fuchsia-400/20 bg-fuchsia-500/10 blur-[0.5px]" />
...
function RoomScene() {
  return (
    <div className="relative h-full overflow-hidden rounded-[2rem] border border-white/10 bg-[#050814] text-white shadow-[0_32px_140px_rgba(0,0,0,0.65)]">
      <div
        className="pointer-events-none absolute inset-0 opacity-95"
        style={{
          backgroundImage:
            'radial-gradient(circle at 50% 18%, rgba(56,189,248,0.26), transparent 24%), radial-gradient(circle at 24% 84%, rgba(168,85,247,0.16), transparent 18%), radial-gradient(circle at 80% 78%, rgba(16,185,129,0.14), transparent 18%), linear-gradient(135deg, rgba(255,255,255,0.05), transparent 34%, rgba(255,255,255,0.03) 68%, transparent 100%)',
        }}
      />
      <div className="relative grid h-full gap-4 p-4 sm:grid-cols-[1fr_0.88fr]">
        <div className="rounded-[1.6rem] border border-white/10 bg-white/5 p-5 backdrop-blur-2xl shadow-[0_24px_100px_rgba(0,0,0,0.45)]">
          ...
          <p className="mt-3 max-w-xl text-sm leading-relaxed text-white/65">
            Start free with a live assessment, then upgrade to a written diagnostic when the workflow needs a decision memo, implementation settings, and a next test.
          </p>
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
